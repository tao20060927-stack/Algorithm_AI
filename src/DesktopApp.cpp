...
if(gold.length&&best&&best.tile!="G")return `金币进入候选集，但最高分是 ${best.tile||"空格"} ${posText(best.realTarget)}：重点看 Iproxy 和 qEff*len 是否压过金币。`;
...
const rows=[...(d.candidates||[])].sort((a,b)=>b.score-a.score).map(c=>`<tr class="${c.selected?"selected ":""}${c.tile=="G"?"gold-row":""}"><td>${c.selected?"* ":""}${c.tile||" "}</td><td>${posText(c.realTarget)}</td><td>${fmt(c.score)}</td><td>${c.deltaR}</td><td>${fmt(c.Iproxy)}</td><td>${c.unknownComponentSum??0}</td><td>${(c.unknownComponents||[]).join("+")||"0"}</td><td>${fmt(c.tailGain)}</td><td>${fmt(c.qEff)}</td><td>${c.pathLen}</td><td>${c.projectedResource}</td></tr>`).join("");
$("debugTable").innerHTML=`<table class="debug-table"><thead><tr><th>目标</th><th>坐标</th><th>score</th><th>deltaR</th><th>Iproxy</th><th>|C|合计</th><th>|C|明细</th><th>tailUB</th><th>qEff</th><th>len</th><th>projR</th></tr></thead><tbody>${rows}</tbody></table>`;
}
)HTML" + LR"HTML(
